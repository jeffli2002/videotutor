...
class MathSolutionScene(Scene):
    def construct(self):
        ...
        # 题目
        question_text = Text("题目: 计算：√(25 + 144)", font_size=28, color=YELLOW)
        ...
        step_content_1_0 = Text("题目要求我们计算 √(25 +", font_size=22, color=WHITE)
        step_content_1_0.next_to(step_title_1, DOWN, buff=0.5)
        self.play(Write(step_content_1_0))
        self.wait(0.8)
        step_content_1_1 = Text("144)，即先将25与144相加，然后对它们的和取平方根。这是一个涉及基本算术运算（加法）和平方根的数学问题，属于初中阶段的基础代数内容。", font_size=22, color=WHITE)
        ...
        step_content_2_0 = Text("根据题意，我们需要先进行加法运算，再进行平方根运算：", font_size=22, color=WHITE)
        ...
        step_content_3_0 = Text("- 先计算括号内的加法： 25 + 144 = 169 -", font_size=22, color=WHITE)
        step_content_3_0.next_to(step_title_3, DOWN, buff=0.5)
        self.play(Write(step_content_3_0))
        self.wait(0.8)
        step_content_3_1 = Text("接着对169开平方： = 13", font_size=22, color=WHITE)
        ...
        step_content_4_0 = Text("最终结果为： = 13 ---", font_size=22, color=WHITE)
