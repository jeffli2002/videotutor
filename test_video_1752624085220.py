...
class MathSolutionScene(Scene):
    def construct(self):
        ...
        # 步骤内容
        step_content = Text("题目要求我们对两个一次多项式进行乘法运算，并将其结果化简为一个标准的二次多项式。      其中第一个多项式是 $3x + 2$，第二个多项式是 $x - 4$。我们需要通过乘法展开并合并同类项来得到最终结果。", 
        ...
        # 步骤内容
        step_content = Text("我们使用乘法分配律（也称为“分配法则”）来进行展开：    $$    (a + b)(c + d) = a \cdot c + a \cdot d + b \cdot c + b \cdot d    $$    应用到本题中：    $$    (3x + 2)(x - 4) = 3x \cdo", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
        ...
        # 步骤内容
        step_content = Text("按照上述公式逐步计算每一项：     - 第一项：$3x \cdot x = 3x^2$      - 第二项：$3x \cdot (-4) = -12x$      - 第三项：$2 \cdot x = 2x$      - 第四项：$2 \cdot (-4) = -8$     将这些结果相加：", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
