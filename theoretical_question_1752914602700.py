...
class SceneScene(Scene):
    def construct(self):
        ...
        for i in range(len(words)):
            if len(current_line) >= max_line_length and words[i] == "，":
        ...
        for i in range(len(explanation_lines)):
            ...
            if i == 0:
                ...
                line.next_to(explanation_group[i-1], DOWN, buff=0.3)
        ...
        # 添加问题文本（如果空间允许）
        if len(explanation_lines) <= 6:
            question_text = Text(
                "问题：请解释三角形的面积公式", 
                font_size=16, 
                color=GRAY
            ).next_to(explanation_group, DOWN, buff=0.5)
