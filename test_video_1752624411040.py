...
class MathSolutionScene(Scene):
    def construct(self):
        ...
        # 步骤内容
        step_content = Text("题目要求我们求出使得等式成立的所有实数解（即方程的根）。      方程为：      $$ x^2 - 5x + 6 = 0 $$      其中，$ a = 1 $, $ b = -5 $, $ c = 6 $。", 
        ...
        # 步骤内容
        step_content = Text("我们需要解的是一个一元二次方程：      $$ x^2 - 5x + 6 = 0 $$", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
        ...
        # 步骤内容
        step_content = Text("我们尝试使用**因式分解法**。      找两个数，使得它们的乘积为常数项 $ c = 6 $，且和为一次项系数 $ b = -5 $。       满足这两个条件的两个数是：-2 和 -3      因为：    - $ (-2) \times (-3) = 6 $    - $ (-2) + ", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
        ...
        # 步骤内容
        step_content = Text("当两个数相乘等于零时，至少有一个因子为零。      所以分别令每个因子为零：     - $ x - 2 = 0 $ → $ x = 2 $    - $ x - 3 = 0 $ → $ x = 3 $     因此，方程的两个解为：      $$ x = 2 \quad \text{或} \q", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
