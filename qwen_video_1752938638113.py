...
class MathSolutionScene(Scene):
    def construct(self):
        ...
        # 显示步骤
        steps = ["题目请用动画讲解三角形勾股定理。","直角三角形有一个角为90度的三角形。","- 2ab = a2 + b2 --- 步骤 6小正方形面积等于 c2 - 具体操作因为小正方形的边长是 c，所以它的面积是 c2 。 - 详细解释通过面积相等，我们得出 c2 = a2 + b2 - 中间结果 c2 = a2 + b2 --- 最终答案 勾股定理的公式为 boxeda2 + b2 = c2 其中，a 和 b 是直角三角形的两条直角边，c 是斜边。 --- 验证过程 以一个常见的直角三角形为例a = 3，b = 4，c = 5。 代入公式验证 32 + 42 = 9 + 16 = 25 = 52 验证成立。 --- 相关数学概念","代数恒等式 (a + b)2 = a2 + 2ab + b2","勾股定理又称毕达哥拉斯定理，是几何学中最基础的定理之一。 ---"]
