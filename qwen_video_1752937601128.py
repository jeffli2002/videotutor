...
class MathSolutionScene(Scene):
    def construct(self):
        ...
        # 显示步骤
        steps = ["题目帮我生成一个三角形面积不变的拉窗帘原理讲解的视频。","具体操作写出三角形面积公式 A = frac12 times text底 times text高","解释面积公式是所有三角形通用的，只要知道底边长度和对应高的长度，就可以计算面积。我们后续将利用这个公式来保持面积不变。","验证结论面积保持不变，符合拉窗帘原理。 --- 相关数学概念 - 三角形面积公式 A = frac12 times text底 times text高 - 比例关系底和高成反比时，面积保持不变 - 几何变换形状变化但面积不变的几何现象 - 函数关系面积是底和高的函数，固定面积时，底和高呈反比例函数关系 --- 常见错误提醒"]
