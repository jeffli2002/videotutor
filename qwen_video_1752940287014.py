...
class MathSolutionScene(Scene):
    def construct(self):
        ...
        # 显示步骤
        steps = ["题目动画演示三角形面积不变的拉窗帘原理。","三角形面积公式 S = frac12 times text底 times text高","混淆高与边长有些学生会误将斜边当作高来计算面积，导致错误。","忽略单位一致性计算面积时要注意单位统一，如底和高都用厘米或米。","不理解拉窗帘含义不清楚拉窗帘是沿平行线移动顶点，从而保持高不变。 --- 通过以上步骤，学生可以直观理解拉窗帘过程中三角形面积不变的数学原理，同时掌握面积计算和几何变换的基本思想。"]
