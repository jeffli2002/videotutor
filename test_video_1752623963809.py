...
class MathSolutionScene(Scene):
    def construct(self):
        ...
        # 步骤内容
        step_content = Text("题目给出的是一个三角形，其中底边长度为8单位，从该底边垂直到底边对应顶点的距离（即高）为6单位。目标是求出这个三角形所围成的平面区域的大小，也就是面积。", 
        ...
        # 步骤内容
        step_content = Text("三角形的面积可以通过以下公式计算：    $$    \text{面积} = \frac{1}{2} \times \text{底边} \times \text{高}    $$    将题目中给出的数据代入公式：    $$    \text{面积} = \frac{1}{2} \times 8 ", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
