...
class MathSolutionScene(Scene):
    def construct(self):
        ...
        step_content_1_0 = Text("题目给出一个关于 x 的线性不等式： 3x - 7 > 14", font_size=22, color=WHITE)
        step_content_1_0.next_to(step_title_1, DOWN, buff=0.5)
        self.play(Write(step_content_1_0))
        self.wait(0.8)
        step_content_1_1 = Text("。我们需要找到所有使得这个不等式成立的 x", font_size=22, color=WHITE)
        step_content_1_1.next_to(step_title_1, DOWN, buff=0.9)
        self.play(Write(step_content_1_1))
        self.wait(0.8)
        step_content_1_2 = Text("值。这需要我们对不等式的性质有一定的了解，尤其是当两边同时除以正数时，不等号方向不变。", font_size=22, color=WHITE)
        ...
        step_content_2_0 = Text("原始不等式为： 3x - 7 > 14", font_size=22, color=WHITE)
        ...
        step_content_3_0 = Text("我们按照以下步骤进行变形： - 首先，把常数项 -7", font_size=22, color=WHITE)
        step_content_3_0.next_to(step_title_3, DOWN, buff=0.5)
        self.play(Write(step_content_3_0))
        self.wait(0.8)
        step_content_3_1 = Text("移到右边，即两边同时加上 7： 3x - 7 + 7 > 14 + 7", font_size=22, color=WHITE)
        step_content_3_1.next_to(step_title_3, DOWN, buff=0.9)
        self.play(Write(step_content_3_1))
        self.wait(0.8)
        step_content_3_2 = Text("化简得： 3x > 21 - 接下来，两边同时除以 3（注意：3", font_size=22, color=WHITE)
        ...
        step_content_4_0 = Text("不等式 3x - 7 > 14 的解集是所有大于 7 的实数，即： x", font_size=22, color=WHITE)
        step_content_4_0.next_to(step_title_4, DOWN, buff=0.5)
        self.play(Write(step_content_4_0))
        self.wait(0.8)
        step_content_4_1 = Text("> 7 ---", font_size=22, color=WHITE)
