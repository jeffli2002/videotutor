...
class MathSolutionScene(Scene):
    def construct(self):
        ...
        # 步骤内容
        step_content = Text("题目要求我们计算 √(25 + 144)，即先将25和144相加，然后对它们的和取平方根。这是一个典型的实数运算问题，重点在于掌握运算顺序和平方根的基本性质。", 
        ...
        # 步骤内容
        step_content = Text("根据题目，数学表达式为：    $$    \sqrt{25 + 144}    $$", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
        ...
        # 步骤内容
        step_content = Text("- 先计算括号内的加法：      $$      25 + 144 = 169      $$    - 然后对169开平方：      $$      \sqrt{169} = 13      $$", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
        ...
        # 步骤内容
        step_content = Text("最终结果是：    $$    \sqrt{25 + 144} = 13    $$ ---", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
