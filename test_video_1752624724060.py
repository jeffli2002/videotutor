...
class MathSolutionScene(Scene):
    def construct(self):
        ...
        # 步骤内容
        step_content = Text("题目要求我们找出使得等式 2x + 5 = 15 成立的 x 值。也就是说，我们需要找到一个数，当它乘以 2 并加上 5 后结果等于 15。", 
        ...
        # 步骤内容
        step_content = Text("已知的数学关系是：    $$    2x + 5 = 15    $$", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
        ...
        # 步骤内容
        step_content = Text("- **第一步操作：消去常数项（+5）**        为了求出 x，首先需要把等式中的常数项 5 消掉。我们可以从等式两边同时减去 5：      $$      2x + 5 - 5 = 15 - 5      $$      化简后得到：      $$      2x = 10      ", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
