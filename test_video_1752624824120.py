...
class MathSolutionScene(Scene):
    def construct(self):
        ...
        # 步骤内容
        step_content = Text("题目给出一个三角形，其底边长度为8个单位，高度为6个单位。要求我们求出这个三角形的面积。这里的“高”是指从底边垂直向上到对边顶点的距离。", 
        ...
        # 步骤内容
        step_content = Text("三角形面积的计算公式为：    $$    \text{面积} = \frac{1}{2} \times \text{底边} \times \text{高}    $$    将题目中给出的数值代入公式：    $$    \text{面积} = \frac{1}{2} \times 8 \time", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
        ...
        # 步骤内容
        step_content = Text("按照运算顺序逐步计算：    - 先计算乘法部分：$8 \times 6 = 48$    - 再乘以 $\frac{1}{2}$：$\frac{1}{2} \times 48 = 24$", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
        ...
        # 步骤内容
        step_content = Text("所以，该三角形的面积是 24 平方单位。 ---", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
