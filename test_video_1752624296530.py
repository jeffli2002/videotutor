...
class MathSolutionScene(Scene):
    def construct(self):
        ...
        # 步骤内容
        step_content = Text("题目给出一个包含加法和平方根的数学表达式：√(25 + 144)，要求我们计算其数值结果。      其中，“√”表示平方根符号，即求某个数的平方等于括号内数值的正数解。", 
        ...
        # 步骤内容
        step_content = Text("原始表达式为：    $$    \sqrt{25 + 144}    $$", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
        ...
        # 步骤内容
        step_content = Text("- 首先计算括号内的加法：      $$      25 + 144 = 169      $$    - 接着对结果开平方：      $$      \sqrt{169} = 13      $$", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
        ...
        # 步骤内容
        step_content = Text("最终计算结果为：    $$    \sqrt{25 + 144} = 13    $$  ---", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
