// 简单的Manim测试脚本
console.log('🎬 简单Manim测试...\n')

async function testSimpleManim() {
  const testQuestion = '帮我用视频动画解释勾股定理'
  
  console.log('📝 测试问题:', testQuestion)
  
  // 构建简单的勾股定理Manim脚本（避免使用weight参数）
...
class SimplePythagoreanScene(Scene):
    def construct(self):
...
`
  ...
  try {
    ...
    const response = await fetch('http://localhost:5001/api/manim_render', {
      ...
      body: JSON.stringify({
        script: manimScript,
        output_name: `simple_pythagorean_${Date.now()}`,
        scene_name: 'SimplePythagoreanScene'
      })
    })
...
// 运行测试
testSimpleManim().catch(console.error) 