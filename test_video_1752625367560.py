...
class MathSolutionScene(Scene):
    def construct(self):
        ...
        # 步骤内容
        step_content = Text("题目要求我们求函数 $ f(x) = 2x + 3 $ 在 $ x = 4 $ 时的值。也就是说，当输入值 $ x $ 为 4 时，我们需要计算出对应的输出值 $ f(4) $。", 
        ...
        # 步骤内容
        step_content = Text("函数表达式为：    $$    f(x) = 2x + 3    $$    将 $ x = 4 $ 代入该表达式中，得到：    $$    f(4) = 2 \cdot 4 + 3    $$", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
        ...
        # 步骤内容
        step_content = Text("- 先进行乘法运算：      $$      2 \cdot 4 = 8      $$    - 然后进行加法运算：      $$      8 + 3 = 11      $$", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
        ...
        # 步骤内容
        step_content = Text("因此，函数 $ f(x) = 2x + 3 $ 在 $ x = 4 $ 时的值为：    $$    f(4) = 11    $$ ---", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
