...
class MathSolutionScene(Scene):
    def construct(self):
        ...
        # 题目
        question_text = Text("题目: 解不等式：3x - 7 > 14", font_size=28, color=YELLOW)
        ...
        step_content_1_0 = Text("题目给出的不等式是 3x - 7 > 14", font_size=22, color=WHITE)
        step_content_1_0.next_to(step_title_1, DOWN, buff=0.5)
        self.play(Write(step_content_1_0))
        self.wait(0.8)
        step_content_1_1 = Text("，要求我们找出所有满足这个不等式的 x 值。", font_size=22, color=WHITE)
        step_content_1_1.next_to(step_title_1, DOWN, buff=0.9)
        self.play(Write(step_content_1_1))
        self.wait(0.8)
        step_content_1_2 = Text("这是一个线性不等式，可以通过基本的代数操作来求解。", font_size=22, color=WHITE)
        ...
        step_content_3_0 = Text("- 第一步运算：将常数项 -7 移到不等式右边，即两边同时加上 7：", font_size=22, color=WHITE)
        step_content_3_0.next_to(step_title_3, DOWN, buff=0.5)
        self.play(Write(step_content_3_0))
        self.wait(0.8)
        step_content_3_1 = Text("3x - 7 + 7 > 14 + 7 化简得： 3x > 21 -", font_size=22, color=WHITE)
        step_content_3_1.next_to(step_title_3, DOWN, buff=0.9)
        self.play(Write(step_content_3_1))
        self.wait(0.8)
        step_content_3_2 = Text("第二步运算：两边同时除以 3（因为 3", font_size=22, color=WHITE)
        ...
        step_content_4_0 = Text("不等式 3x - 7 > 14 的解集是： x > 7 ---", font_size=22, color=WHITE)
