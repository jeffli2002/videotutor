...
class MathSolutionScene(Scene):
    def construct(self):
        ...
        # 步骤内容
        step_content = Text("题目要求我们找出所有使得表达式 $ 3x - 7 $ 大于 14 的 $ x $ 值。这是一个基础的一元一次不等式问题，只需要进行基本的代数操作即可求解。", 
        ...
        # 步骤内容
        step_content = Text("我们按照以下步骤来解这个不等式：    - 第一步：将常数项移到不等号右边：      $$      3x > 14 + 7      $$      $$      3x > 21      $$    - 第二步：两边同时除以 3（因为 3 是正数，不改变不等号方向）：      $$    ", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
        ...
        # 步骤内容
        step_content = Text("不等式的解集是所有大于 7 的实数。 ---", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
