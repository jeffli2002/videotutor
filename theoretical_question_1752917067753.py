...
class SceneScene(Scene):
    def construct(self):
        ...
        # 在右侧区域显示解释内容 - 优化布局，确保在显示区域内
        explanation_lines = []
        solution_text = "三角形的面积公式是底边乘以高再除以二。这个公式适用于所有类型的三角形。"
        max_line_length = 20  # 减少每行字符数，确保不超出显示区
        
        # 分割解答文本为多行
        current_line = ""
        words = list(solution_text)
        for i in range(len(words)):
            if len(current_line) >= max_line_length and words[i] in ["，", "。", "；"]:
                explanation_lines.append(current_line + words[i])
                current_line = ""
            else:
        ...
        for i in range(len(explanation_lines)):
            ...
            if i == 0:
                line.move_to(right_area.get_top() + DOWN * 0.6)  # 调整起始位置
            else:
                line.next_to(explanation_group[i-1], DOWN, buff=0.25)  # 减少行间距
