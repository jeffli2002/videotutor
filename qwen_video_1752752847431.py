...
class MathSolutionScene(Scene):
    def construct(self):
        ...
        # 显示步骤
        steps = ["题目帮我生成一个三角形面积的拉窗帘原理的演示视频","仔细阅读题目，理解题意","找出已知条件和未知量","选择合适的数学方法","逐步计算求解","检验答案的合理性 建议 由于当前网络连接问题，建议 - 确认题目是否完整 - 检查是否包含所有必要的数字和条件 - 稍后重试获取完整的AI解答 注这是简化的本地解答，完整的AI解答请稍后重试。"]
