...
class MathSolutionScene(Scene):
    def construct(self):
        ...
        # 步骤内容
        step_content = Text("题目给出的是一个线性不等式：$3x - 7 > 14$。      要求我们找出所有使得这个不等式成立的 $x$ 值。      解题的关键在于进行正确的代数操作，保持不等号方向不变（除非乘以或除以负数）。", 
        ...
        # 步骤内容
        step_content = Text("我们按照标准的代数方法逐步求解：     - 第一步：将常数项移到不等式的右边，加7到两边：      $$      3x - 7 + 7 > 14 + 7      $$      化简得：      $$      3x > 21      $$     - 第二步：两边同时除以3（正数，不改", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
        ...
        # 步骤内容
        step_content = Text("不等式 $3x - 7 > 14$ 的解集是所有大于7的实数。  ---", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
