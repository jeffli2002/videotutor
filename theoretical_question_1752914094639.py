...
class SceneScene(Scene):
    def construct(self):
        ...
        # 标题 - 使用较小的字体避免占用太多空间
        title = Text("请解释导数的几何意义", font_size=32, color=BLUE).to_edge(UP, buff=0.3)
        ...
        explanation_lines = []
        const solutionText = "导数的几何意义是函数图像在某一点的切线斜率。当函数在某点可导时，该点的导数就是函数图像在该点切线的斜率。这个斜率反映了函数在该点的瞬时变化率，正斜率表示函数在该点递增，负斜率表示函数在该点递减，零斜率表示函数在该点有极值。"
