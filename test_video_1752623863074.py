...
class MathSolutionScene(Scene):
    def construct(self):
        ...
        # 步骤内容
        step_content = Text("题目给出一个等式：$ 2x + 5 = 15 $。      其中，$ x $ 是未知数，我们的目标是求出 $ x $ 的值，使得这个等式成立。", 
        ...
        # 步骤内容
        step_content = Text("题目已经直接给出了方程：      $$    2x + 5 = 15    $$", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
        ...
        # 步骤内容
        step_content = Text("我们需要通过代数方法解这个方程。具体步骤如下：     - **步骤1：移项**        将常数项 $ +5 $ 移到等号右边，变为减法：        $$      2x = 15 - 5      $$      $$      2x = 10      $$     - **步骤2：消", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
        ...
        # 步骤内容
        step_content = Text("解得未知数 $ x $ 的值为：    $$    x = 5    $$  ---", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
