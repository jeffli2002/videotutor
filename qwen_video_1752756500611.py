...
class MathSolutionScene(Scene):
    def construct(self):
        ...
        # 显示步骤
        steps = ["题目解不等式3x - 7 14","首先，我们需要将方程中的常数项移到等号右边 2x + 5 = 15 2x = 15 - 5 2x = 10 解释通过移项，我们将未知数项和常数项分离","然后，我们通过除以系数来求解x 2x = 10 x = 10 ÷ 2 x = 5 解释为了求解x，我们需要消除x的系数2","详细解释（为什么要这样做）","中间结果（显示计算过程） 例如"]
