...
class MathSolutionScene(Scene):
    def construct(self):
        ...
        # 步骤内容
        step_content = Text("题目要求我们找出满足等式 2x + 5 = 15 的 x 值。其中，“2x”表示 x 的两倍，“+5”表示再加上5，整个表达式等于15。我们需要找到这个 x 的具体数值。", 
        ...
        # 步骤内容
        step_content = Text("根据题目直接给出的信息，我们可以写出原始方程：    $$    2x + 5 = 15    $$", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
        ...
        # 步骤内容
        step_content = Text("- **第一步操作：消去常数项（+5）**        为了求出 x，首先需要把等式中的常数项“+5”移到右边。根据等式性质，两边同时减去5：      $$      2x + 5 - 5 = 15 - 5      $$      化简后得到：      $$      2x = 10    ", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
        ...
        # 步骤内容
        step_content = Text("经过上述运算，我们得到了未知数 x 的值为 5。  ---", 
                          font_size=24, color=WHITE, line_spacing=1.2)
        step_content.next_to(step_title, DOWN, buff=0.8)
        step_content.to_edge(LEFT, buff=1)
        self.play(Write(step_content))
